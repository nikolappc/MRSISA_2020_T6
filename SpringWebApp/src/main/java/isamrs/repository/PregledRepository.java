...
public interface PregledRepository extends JpaRepository<Pregled, Integer> {
	...
	@Query("SELECT p FROM Klinika k JOIN k.pregledi p WHERE p.sala = null and k.id = ?1")
	public List<Pregled> findZahteve(Integer idKlinike);
	...
	@Query("SELECT p FROM Pregled p WHERE p.zdravstveniKarton.id = ?1 AND p.sala != null")
	public List<Pregled> findByIdKartonLek(Integer id);
	
	@Query("SELECT p FROM Pregled p WHERE p.sala = ?1 and p.termin.kraj >= ?2")
	public List<Pregled> findBySala(Sala sala, Date d);
	...
	@Query("SELECT p FROM Pregled p WHERE p.lekar.id = ?2 and p.zdravstveniKarton.id = ?1")
	public List<Pregled> checkKarton(Integer idKarton, Integer idLekar);
}
